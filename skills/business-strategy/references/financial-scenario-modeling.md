...
Financial scenarios should be consistent with the selected wedge, GTM motion, market size, and expansion sequence.
...
Tie expansion to the strategy sequence:

```text
initial wedge → first paid use case → organizational expansion → platform / additional products
```
...
Then test whether market size, sales capacity, onboarding, retention, and expansion can support that.

## Strategy-linked downside

Model at least one scenario where a strategic assumption fails, not only where every variable is modestly worse.

Examples:

- incumbent bundles the core feature
- OSS adoption does not convert to enterprise
- usage grows but price per unit collapses
- customer behavior shifts to a different workflow

This exposes whether the business model is robust to the same risks identified in strategy stress testing.

## Valuation scenarios

After revenue scenarios, do not apply one fixed multiple to all cases. Retrieve current context-appropriate comparables when valuation matters and explain why different scenarios warrant different ranges.